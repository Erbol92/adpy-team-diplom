...
from app.bot.core import longpoll, send_start_message, geo_user, confirm_choose, send_message_event_answer
from app.database.orm_query import orm_check_user_in_database, orm_add_user, init_database
...
async def main():
    ...
    for event in longpoll.listen():
        ...
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            is_searched = await orm_check_user_searched(user_vk_id)

            if is_searched and menu[user_vk_id].pages and event.object.message.get('text'):
            ...
            match payload:
                ...
                case 'like':
                    ...
                    await send_message_event_answer(event.object.event_id, user_vk_id, event.obj.peer_id, text)
                case 'dislike':
            ...
            if payload not in ['confirm', 'reset', 'like']:
                await send_message_event_answer(event.object.event_id, user_vk_id, event.obj.peer_id)
            print(event.object)
